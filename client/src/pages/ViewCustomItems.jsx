import React, { useEffect, useState } from "react";
import customItemAPI from "../services/customItemAPI";
import CustomItemCard from "../components/CustomItemCard";
import "../App.css";

const ViewCustomItems = () => {
  const [customItems, setCustomItems] = useState();
  const customItemsDummyData = [
    {
      title: "Spaghetti Carbonara",
      description:
        "A classic Italian pasta dish made with eggs, cheese, pancetta, and pepper.",
      imgURL:
        "https://plus.unsplash.com/premium_photo-1691948106030-d5e76d461b14?w=700&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8U3BhZ2hldHRpJTIwQ2FyYm9uYXJhfGVufDB8fDB8fHww",
      preparationTime: 1200,
      servings: "4",
      ingredients:
        "{400g spaghetti, 3 large eggs, 100g Pecorino Romano, 150g diced pancetta, black pepper to taste, salt to taste}",
      instructions:
        "{Cook spaghetti, Fry pancetta, Whisk eggs and cheese, Combine all and serve hot}",
    },
    {
      title: "Chicken Tikka Masala",
      description:
        "A flavorful Indian dish made with marinated chicken in a spiced tomato sauce.",
      imgURL:
        "https://plus.unsplash.com/premium_photo-1670191894591-bc501a33de0d?q=80&w=2576&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      preparationTime: 3600,
      servings: "4",
      ingredients:
        "{500g boneless, skinless chicken, 200g plain yogurt, 1 tsp garam masala, 1 tsp cumin, 400g tomato sauce, 100ml cream, 1 chopped onion, 2 cloves minced garlic, 1 inch grated ginger}",
      instructions:
        "{Marinate chicken in yogurt and spices, Cook onion, garlic, and ginger, Add chicken and sauce, simmer}",
    },
    {
      title: "Vegetable Stir Fry",
      description:
        "A quick and healthy dish with mixed vegetables and soy sauce.",
      imgURL:
        "https://images.unsplash.com/photo-1464500650248-1a4b45debb9f?q=80&w=2598&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",

      preparationTime: 900,
      servings: "2",
      ingredients:
        "{2 cups mixed vegetables (bell peppers, broccoli, carrots), 2 tbsp soy sauce, 1 clove minced garlic, 1 inch grated ginger, 1 tbsp sesame oil}",
      instructions:
        "{Heat oil, sauté garlic and ginger, add vegetables, stir-fry for 5 minutes, add soy sauce}",
    },
    {
      title: "Chocolate Chip Cookies",
      description:
        "Delicious cookies filled with chocolate chips, perfect for any occasion.",
      imgURL:
        "https://plus.unsplash.com/premium_photo-1672073871234-7f2de18b9e5d?q=80&w=2574&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      preparationTime: 1800,
      servings: "12",
      ingredients:
        "{2 cups all-purpose flour, 1 cup brown sugar, 1 cup softened butter, 2 eggs, 1 cup chocolate chips, 1 tsp baking soda, 1 tsp vanilla, 1/2 tsp salt}",
      instructions:
        "{Mix ingredients, scoop dough onto baking sheet, bake at 350°F for 10-12 minutes}",
    },
    {
      title: "Caesar Salad",
      description: "A fresh salad with romaine, croutons, and Caesar dressing.",
      imgURL:
        "https://plus.unsplash.com/premium_photo-1700089483464-4f76cc3d360b?q=80&w=2574&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      preparationTime: 600,
      servings: "2",
      ingredients:
        "{1 head chopped romaine, 1 cup croutons, 1/2 cup grated parmesan, 1/4 cup Caesar dressing, black pepper to taste}",
      instructions:
        "{Combine all ingredients in a bowl, toss, and serve immediately}",
    },
    {
      title: "Beef Tacos",
      description:
        "Flavorful tacos filled with seasoned beef and topped with fresh ingredients.",
      imgURL:
        "https://plus.unsplash.com/premium_photo-1661730314652-911662c0d86e?q=80&w=2670&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",

      preparationTime: 1500,
      servings: "4",
      ingredients:
        "{500g ground beef, 8 taco shells, 1 cup shredded lettuce, 1 diced tomato, 1 cup shredded cheese, 2 tbsp taco spices}",
      instructions:
        "{Cook beef with spices, fill taco shells, and top with lettuce, tomato, and cheese}",
    },
    {
      title: "Lemonade",
      description:
        "A refreshing drink made with fresh lemons, sugar, and water.",
      imgURL:
        "https://images.unsplash.com/photo-1523677011781-c91d1bbe2f9e?q=80&w=2586&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",

      preparationTime: 600,
      servings: "4",
      ingredients:
        "{4 juiced lemons, 1 cup sugar, 4 cups water, ice as needed, mint for garnish}",
      instructions:
        "{Mix lemon juice, sugar, and water, Serve over ice and garnish with mint}",
    },
    {
      title: "Pancakes",
      description:
        "Fluffy pancakes perfect for breakfast, served with syrup and berries.",
      imgURL:
        "https://plus.unsplash.com/premium_photo-1663854478296-dd00b6257021?q=80&w=2574&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",

      preparationTime: 1200,
      servings: "4",
      ingredients:
        "{1 cup flour, 1 cup milk, 1 egg, 2 tsp baking powder, 2 tbsp sugar, butter for cooking}",
      instructions:
        "{Mix ingredients, pour batter onto griddle, cook until bubbles form, flip and serve}",
    },
    {
      title: "Minestrone Soup",
      description: "A hearty Italian soup filled with vegetables and pasta.",
      imgURL:
        "https://plus.unsplash.com/premium_photo-1664391935474-f1e502d3ad61?q=80&w=2710&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      preparationTime: 2400,
      servings: "6",
      ingredients:
        "{4 cups vegetable broth, 1 can beans (drained), 1 cup small pasta, 2 cups diced vegetables (carrots, celery, zucchini), 1 tsp Italian seasoning, salt to taste}",
      instructions:
        "{Simmer broth and vegetables, add pasta and beans, cook until pasta is tender}",
    },
    {
      title: "Caprese Salad",
      description:
        "A simple Italian salad made with fresh mozzarella, tomatoes, and basil.",
      imgURL:
        "https://plus.unsplash.com/premium_photo-1677619680535-c3d453823b9e?q=80&w=2670&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      preparationTime: 600,
      servings: "2",
      ingredients:
        "{200g sliced mozzarella, 2 large sliced tomatoes, a handful of basil, 2 tbsp olive oil, 1 tbsp balsamic vinegar, salt to taste}",
      instructions:
        "{Layer mozzarella and tomatoes, sprinkle with basil, drizzle with oil and vinegar}",
    },
  ];

  useEffect(() => {
    const fetchCustomItems = async () => {
      const data = await customItemAPI.getAllCustomItems();
      setCustomItems(data);
      console.log(data);
    };

    fetchCustomItems();
  }, []);

  return (
    <>
      <div className="flex justify-center py-20 bg-slate-900/5">
        <h1
          className="text-white text-5xl font-thin p-2 font-mono italic"
          style={{ textShadow: "4px 4px 10px black" }}
        >
          All Listed Recipes
        </h1>
      </div>
      <div className="flex flex-row space-x-2 overflow-y-scroll p-2">
        {customItemsDummyData.map((customItem) => (
          <CustomItemCard
            title={customItem.title}
            imgURL={customItem.imgURL}
            servings={customItem.servings}
            description={customItem.description}
            ingredients={customItem.ingredients}
            instructions={customItem.instructions}
          />
        ))}
      </div>
    </>
  );
};

export default ViewCustomItems;
